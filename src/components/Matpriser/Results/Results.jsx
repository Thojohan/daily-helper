...
function sortRes(res, sortWord) {
  ...
  if (sortWord === "kalorier")
    return res.toSorted(
      (a, b) =>
        a.nutrition.reduce(
          (acc, el) => (el.code === "energi_kcal" ? acc + el.amount : acc),
          0
        ) -
        b.nutrition.reduce(
          (acc, el) => (el.code === "energi_kcal" ? acc + el.amount : acc),
...
function Results({ searchRes, selectedShops, tickedShops, sortBy }) {
  ...
  const sorted = sortRes(searchRes.data, sortBy).filter((el) =>
    tickedShops
      ? selectedShops
          .map((e) => (e.toLowerCase().includes("coop") ? "COOP_NO" : e))
          .find((e) => e === el.store.code || el.store.code === "ODA_NO")
      : el
  );
